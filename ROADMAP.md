...
| 0.4 | `ipc` | Protocol message types: `HostToContainer` and `ContainerToHost` enums. Frame codec: 4-byte length prefix + JSON payload. Unix socket server (host side) and client (container side) with tokio. Handshake: container connects, sends `Ready`, host sends `Init`. Unit tests for codec roundtrip, connection lifecycle. | **Done** |
